...
<div id="loginpanel">
	...
	<div id="loginForm">
		...
		<p>
			<input type="checkbox" onkeydown="EnterTo('SubmitL',event)" value="true" name="Auto" id="Auto" />&nbsp;下次自动登录
		</p>
		<p>
			<input type="button" onclick="Login()" value="登录" class="subbutton" name="SubmitL" />
			<a href="/help/getcode.aspx">忘了密码?</a>
		</p>
	</div>
...
function Login(){
	...
	$.post('<%=Url.Action("ProcessLogin","Account") %>',{"e":U,"p":P,"a":$("#Auto").attr("checked")},function(r){
		if(r!="false"){
			$h("#linkList2",r);;
		}
		else{
