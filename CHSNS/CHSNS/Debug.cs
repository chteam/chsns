...
namespace CHSNS {
	...
	public class Debug {
		static void SaveTextFile(string path, string text) {
			using (System.IO.StreamWriter sw = new System.IO.StreamWriter(path, true)) { sw.WriteLine(text); }
		}
		/// <summary>
		...
		static public void Trace(string info) {
			string fn = string.Format("{0}\\Log\\{1}.txt", HttpContext.Current.Server.MapPath("/"), DateTime.Now.ToString("yyyyMMdd"));
			//SaveTextFile1(fn);
			if (CHUser.IsLogin)
				SaveTextFile(fn, string.Format("[{0}][用户:{2}] : {1}", DateTime.Now.ToLongTimeString(), info, HttpContext.Current.Session["userid"].ToString()));
			else
				SaveTextFile(fn, string.Format("[{0}][IP:{2},代理IP:{3}] : {1}",
