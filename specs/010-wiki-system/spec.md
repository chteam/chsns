# Feature Specification: Wiki 知识库系统

**Feature Branch**: `010-wiki-system`  
**Created**: 2025-11-14  
**Status**: Draft  
**Input**: Wiki 知识库系统：创建 Wiki 词条、编辑词条内容、词条版本历史、词条分类标签、词条搜索、词条链接引用、协作编辑、词条审核
...
- **EditSummary** (string, 200 字符, 可选): 编辑摘要
- **索引**: (WikiId, VersionNumber) 用于版本查询

#### WikiCategory (Wiki 分类实体)

- **Id** (long, 主键): 分类 ID
- **Name** (string, 50 字符): 分类名称
- **Description** (string, 200 字符, 可选): 分类描述
...
- **TargetWikiTitle** (string, 200 字符): 目标词条标题
- **LinkText** (string, 200 字符, 可选): 链接显示文本
- **CreateTime** (DateTime): 创建时间
