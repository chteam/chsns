...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <div class="ch_content">
        ...
        <ol id="UserListItems" class="userlist">
            <%Html.RenderPartial("RequestList", View.Items); %>
        </ol>
        <div id="PageDown" class="page">
        </div>
    </div>
    <%=Html.Hidden("PageCount", View.Items.TotalPages)%>
    <%=Html.Hidden("NowPage", View.Items.CurrentPage)%>
    <%=Html.Hidden("EveryPage", View.Items.PageSize)%>
    ...
    var setpage = function(p) {
        $.post('<%=Url.Action("RequestList") %>', {"p":p,"userid":<%=View.Profile.UserId %>}, function(r) {
            $h("#UserListItems",r);
            pagefun();
        });
    };
