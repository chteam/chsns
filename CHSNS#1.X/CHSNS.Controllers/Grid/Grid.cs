...
namespace CHSNS.Helper
{
	...
	public class Grid<T> : GridBase<T> where T : class
	{
        ...
        private readonly string _paginationSingleFormat = "当前为第{0}页,共有{1}页 | ";
		...
		protected override void RenderGridEnd(bool isEmpty)
		{
			...
			if(!isEmpty)
			{
				var pagination = Items as PagedList<T>;
				if (pagination != null) {
        ...
        /// Eg "上一页 [1]  2 下一页  最末页  当前为第1页,共有2页"
		/// </summary>
		/// <param name="pagedList"></param>
		protected virtual void RenderPagination(PagedList<T> pagedList)
		{
			var builder = new StringBuilder();
			builder.Append("<div class='pagination'>");
			builder.Append("<span class='paginationLeft'>");
            //当前为第{0}页,共有{1}页
            builder.AppendFormat(_paginationSingleFormat, pagedList.CurrentPage, pagedList.TotalPages);
            ...
            if (pagedList.TotalPages > 1) {
                ...
                if (pagedList.HasPreviousPage)
                    builder.Append(CreatePageLink(pagedList.CurrentPage - 1, _paginationPrev));
                else
                    builder.Append(_paginationPrev);
                for (int i = 2; i <= 12; i++)
                    if ((pagedList.CurrentPage + i - 7) >= 1 && (pagedList.CurrentPage + i - 7) <= pagedList.TotalPages)
                        if (7 == i)
                            builder.Append(string.Format(" [{0}] ", pagedList.CurrentPage));
                        else
                            builder.Append(CreatePageLink(pagedList.CurrentPage + i - 7,(pagedList.CurrentPage + i - 7).ToString()));
                if (pagedList.HasNextPage)
                    builder.Append(CreatePageLink(pagedList.CurrentPage + 1, _paginationNext));
                else builder.Append(_paginationNext);
                if (pagedList.CurrentPage != pagedList.TotalPages)
                    builder.Append(CreatePageLink(pagedList.TotalPages, _paginationLast));
            }
