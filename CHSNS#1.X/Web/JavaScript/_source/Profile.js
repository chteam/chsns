...
/* Sys.Application.add_init(function() {
 var h=$find("Profile_AccordionExtender");
 h.add_selectedIndexChanging(function(sender,e){
 	var p=h.get_Pane(e.get_oldIndex());
 	if(!p||!p.header.id)return;
 	var id=p.header.id;
 	 if($get(id+"_slider"))
	if($get(id+"_old").value!=$get(id+"_slider").value) ChAlumna.Profile.ShowLevel_Edit(id,$get(id+"_slider").value,function(){$get(id+"_old").value=$get(id+"_slider").value;},onfail);
		});
		});*/
}