...
namespace CHSNS.Helper
{
	...
	public class Grid<T> : GridBase<T> where T : class
	{
		...
		protected virtual void RenderPagination(PagedList<T> pagedList)
		{
			...
			{
				builder.AppendFormat(_paginationSingleFormat, 1, pagedList.TotalPages);
			}
			else
			{
				builder.AppendFormat(_paginationFormat, 1, pagedList.TotalCount, pagedList.TotalPages);
			}
			builder.Append("</span>");
			...
			if (pagedList.CurrentPage < lastPage)
			{
